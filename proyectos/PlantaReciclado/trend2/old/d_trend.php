...
if(strpos(strtolower($refer),'info_generador.php')>0){
	$IMG_W=580;
	$IMG_H=240;
}else{
	$IMG_W=1200;
...
echo "<tr><td>";
	...
	echo "<FORM NAME='myForm' ACTION='$accion' METHOD='POST'>";
    ...
    echo " hh <input id='hini' style='font-size:xx-small' type=text maxlength='2' size=2 name='hini' onchange='return ValidarHora();' value=0>";
    echo " mm <input id='mini' style='font-size:xx-small' type=text maxlength='2' size=2 name='mini' onchange='return ValidarHora();' value=0>";
    echo "&nbsp;&nbsp;";
	echo "Fecha Fin";
	$calendario=new Calendar("it");
	$calendario->grandezza_carattere="Small";
    ...
    echo " hh <input id='hfin' style='font-size:xx-small' type=text maxlength='2' size=2 name='hfin' onchange='return ValidarHora();' value=$hfinaux>";
    echo " mm <input id='mfin' style='font-size:xx-small' type=text maxlength='2' size=2 name='mfin' onchange='return ValidarHora();' value=$mfinaux>";
    echo "<input type='image' src='/images/flash1.png' alt='Consultar' width='32' height='32' name='query_por_fecha' value='1' title='Consultar por fechas utilizando el calendario'>\n";
    echo "<BR>";
echo "</tr></td>";
echo "</table>";
...
echo "<tr><td>";
	if($USAR_LUPA)
        	echo "<input type=image src='/images/adjust.png' width='32' height='32' name='query' value='1' title='Ajustar' onclick='Submit()'></input>\n";
	echo "Time Span: <select name='span_combo' languaje=javascript onchange='return document.myForm.submit()'>\n";
	...
	echo "<option value='3' $selected[3]>1 hour</option>\n";
	echo "<option value='4' $selected[4]>6 hours</option>\n";
	echo "<option value='5' $selected[5]>12 hours</option>\n";
	echo "<option value='6' $selected[6]>1 day</option>\n";
	echo "<option value='7' $selected[7]>10 day</option>\n";
	echo "<option value='8' $selected[8]>1 month</option>\n";
	echo "</select>";
	echo "&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp";

echo "</td><td>";
        echo "<input type='image' src='/images/zoom_in.png' width='32' height='32' name='zoom_in' value='1' title='Zoom in'>\n";
echo "</td><td>";
	echo "<input type='image' src='/images/zoom_out.png' name='zoom_out' width='32' height='32' value='1' title='Zoom out'>\n";
echo "</td><td>";
        echo "<input type='image' src='/images/shift_left.png'  width='32' height='32'   name='shift_rigth' value='1' title='Retroceder en el tiempo'>\n";
echo "</td><td>";
        echo "<input type='image' src='/images/shift_rigth.png'  width='32' height='32'  name='shift_left' value='1' title='Avanzar en el tiempo'>\n";
echo "</td><td>";
        echo "<input type='image' src='/images/exportbd.png'  width='32' height='32' name='exportar' value='1' title='Exportar datos'>\n";
echo "</td>";
echo "<td>";
        echo "<input type='image' src='/images/export.png'  width='32' height='32' name='exportar_grafica' value='1' title='Exportar valores de la gr·fica'>\n";
echo "</td>";
...
if($tag_selector_get) // Armo el combo del selector de tags
{
	if($tipo_tags=="a") // analogicos
		$twhere=" where id_tag_types <> 1";
	else if($tipo_tags=="d") // discretos
		$twhere=" where id_tag_types = 1 ";
	else
		$twhere="";
	$query ="select tagname from t_tags $twhere AND active=1 order by tagname asc";
	...
	{
		echo "<td>\n";
		echo "<select name='multi_selector_$i'>\n";
	...
	if($agrupar)
		...
		$ckecked_agrupar="";
        echo "Agrupar graficas<input type='checkbox' $ckecked_agrupar name='agrupar' value='1' title='Agrupar Tags'></input>\n";
	echo "</td>";
	echo "<td>";
        echo "<input type='image' src='/images/check.JPG'  width='32' height='32' name='seleccion_multiple' value='1' title='Seleccion Multiple'>\n";
	echo "</td>";
	echo "</tr></table>";
}
// Para volver
...
{
	Exportar($tagnames);
	echo "<br><a href='$volver_a'><img alt='Inicio'src='/images/go-home.png' width='32' height='32' border=0 title='Volver a pagina de inicio'></a> Inicio";
	die();
}
$valores=array();
...
{
    echo '
    <div id="placeholder" style="width:'.$IMG_W.'px;height:'.$IMG_H.'px;"></div> 
	...
	if(series.miny > 0){
        ...
        var options = {
                ...
                grid: { hoverable: true, clickable: true, color: "#000000", backgroundColor:"rgb(230,230,230)", borderColor:"#000000", tickColor:"rgb(210,210,210)"},
...
//			$graph->SetDataColors(array('red', 'blue','green','magenta','yellow')); 
	...
	{
	    echo '
	    <div id="placeholder'.$pp.'" style="width:'.$IMG_W.'px;height:'.$IMG_H.'px;"></div> 
	    <script type="text/javascript"> 
	   (function($){
          $(function () {
           // generate data set from a parametric function with a fractal
